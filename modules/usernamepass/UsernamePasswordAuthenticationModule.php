 ...
 */
class UsernamePasswordAuthenticationModule extends \StartupAPI\AuthenticationModule
{
	...
	public static function getModulesLogo($size = 100) {
		if ($size == 100) {
			return \StartupAPI\UserConfig::$USERSROOTURL . '/modules/usernamepass/images/logo_100x.png';
		}
	}
	...
	public function getUserCredentials($user)
	{
		...
		if ($stmt = $db->prepare('SELECT username FROM u_users WHERE id = ?'))
		{
			...
			if (!is_null($username))
			{
				return new UsernamePasswordAuthenticationModule\UsernamePassUserCredentials($username);
			}
		}
		else
		{
	...
	public function renderLoginForm($template_info, $action)
	{
		...
		return \StartupAPI\StartupAPI::$template->render(
			"@startupapi/modules/usernamepass/login_form.html.twig",
			$template_info
		);
	}
	...
	public function renderRegistrationForm($template_info, $full = false, $action = null, $errors = null, $data = null)
	{
		...
		return \StartupAPI\StartupAPI::$template->render(
			"@startupapi/modules/usernamepass/registration_form.html.twig",
			$template_info
		);
	}
	...
	public function renderEditUserForm($template_info, $action, $errors, $user, $data)
	{
		...
		return \StartupAPI\StartupAPI::$template->render(
			"@startupapi/modules/usernamepass/edit_user_form.html.twig",
			$template_info
		);
	}
	...
	{
		$remember = \StartupAPI\UserConfig::$allowRememberMe && array_key_exists('remember', $data);

		$user = \StartupAPI\User::getUserByUsernamePassword($data['username'], $data['pass']);
	...
	{
		$remember = \StartupAPI\UserConfig::$allowRememberMe &&
			\StartupAPI\UserConfig::$rememberUserOnRegistration;
		...
		if (count($errors) > 0)
		{
			throw new \StartupAPI\Exceptions\Authentication\InputValidationException(
				'Validation failed', 0, $errors
			);
		}

		if (count(\StartupAPI\User::getUsersByEmailOrUsername($username)) > 0 ) {
			$errors['username'][] = "This username is already used, please pick another one";
		}
		if (count(\StartupAPI\User::getUsersByEmailOrUsername($email)) > 0 ) {
			$errors['email'][] = "This email is already used by another user, please enter another email address.";
		}

		if (count($errors) > 0)
		{
			throw new \StartupAPI\Exceptions\Authentication\ExistingUserException('User already exists', 0, $errors);
		}

		// ok, let's create a user
		$user = \StartupAPI\User::createNew($name, $username, $email, $data['pass']);
	...
	public function processEditUser($user, $data)
	{
		...
		if (!$has_username) {
			...
			if (!is_null($username)) {
				$existing_users = \StartupAPI\User::getUsersByEmailOrUsername($username);
		...
		if (count($errors) > 0)
		{
			throw new \StartupAPI\Exceptions\Authentication\InputValidationException('Validation failed', 0, $errors);
		}
