 ...
 */
class FacebookAuthenticationModule extends \StartupAPI\AuthenticationModule {
	...
	public static function getModulesLogo($size = 100) {
		if ($size == 100) {
			return \StartupAPI\UserConfig::$USERSROOTURL . '/modules/facebook/images/logo_100x.png';
		}
	}
	...
	public function getUserCredentials($user) {
		...
		if ($stmt = $db->prepare('SELECT fb_id FROM u_users WHERE id = ?')) {
			...
			if (!is_null($fb_id)) {
				return new FacebookAuthenticationModule\FacebookUserCredentials($fb_id);
			}
		} else {
	...
	public function renderLoginForm($template_info, $action) {
		if (is_null($action)) {
			$action = \StartupAPI\UserConfig::$USERSROOTURL . '/login.php?module=' . $this->getID();
		}
	...
	public function renderAutoLogoutForm($template_info) {
		...
		return \StartupAPI\StartupAPI::$template->display("@startupapi/modules/facebook/auto_logout_form.html.twig", $template_info);
	}
	...
	private function renderForm($template_info, $action, $form) {
		...
		return \StartupAPI\StartupAPI::$template->render(
			"@startupapi/modules/facebook/forms.html.twig",
			$template_info
		);
	}
	...
	public function renderRegistrationForm($template_info, $full = false, $action = null, $errors = null, $data = null) {
		if (is_null($action)) {
			$action = \StartupAPI\UserConfig::$USERSROOTURL . '/register.php?module=' . $this->getID();
		}
	...
	public function renderEditUserForm($template_info, $action, $errors, $user, $data) {
		...
		if (is_null($fb_id)) {
			...
			try {
				...
				\StartupAPI\UserTools::debug("Can't get /me API data: " . $e);
				return;
			}
			...
			return \StartupAPI\StartupAPI::$template->render(
				"@startupapi/modules/facebook/edit_user_form.html.twig",
				$template_info
			);
		}
	}
	...
	public function processAutoLogin() {
		\StartupAPI\UserTools::debug('Automatic login start');
	...
	public function getAutoLogoutURL($return) {
		return \StartupAPI\UserConfig::$USERSROOTFULLURL . '/modules/facebook/logout.php';
	}
	...
	public function processLogin($post_data, &$remember, $auto = false) {
		\StartupAPI\UserTools::debug('processLogin start');
		$remember = $this->remember;

		try {
			$fbuser = $this->sdk->getUser();
		} catch (\FacebookApiException $e) {
			\StartupAPI\UserTools::debug("Can't get Facebook user");
			return null;
		}

		\StartupAPI\UserTools::debug('Facebook user id: ' . $fbuser);
		...
		if ($fbuser == 0) {
			...
			throw new \StartupAPI\Exceptions\Authentication\InputValidationException('No facebook user id', 0, $errors);
		}

		try {
			$permissions = $this->api('/me/permissions');
			\StartupAPI\UserTools::debug('User permissions: ' . var_export($permissions, true));
			foreach ($this->permissions as $perm) {
				if (!array_key_exists($perm, $permissions['data'][0]) || $permissions['data'][0][$perm] !== 1) {
					...
					\StartupAPI\UserTools::debug("Can't login - not enough permissions granted");
					return null;
				}
			}
		} catch (\StartupAPI\Exception $e) {
			\StartupAPI\UserTools::debug("Can't get FB /me/permissions API data: " . $e);
			return null;
		}

		$user = \StartupAPI\User::getUserByFacebookID($fbuser);
		...
		if (!is_null($user)) {
			...
			\StartupAPI\UserTools::debug('Auto-logged-in, not registering');
			return null;
		} else {
	...
	public function processRegistration($post_data, &$remember) {
		\StartupAPI\UserTools::debug('processRegistration start');
		$remember = $this->remember;
		...
		if ($fbuser == 0) {
			$errors['fbuserid'][] = 'No Facebook id is passed';
			throw new Exceptions\Authentication\InputValidationException('No facebook user id', 0, $errors);
		}

		// checking if user with this Facebook ID already exists and if so, then logs them in
		$existing_user = \StartupAPI\User::getUserByFacebookID($fbuser);
		if (!is_null($existing_user)) {
			\StartupAPI\UserTools::debug(
				'processRegistration - existing user: ' . $existing_user->getID()
			);
		...
		try {
			$user = \StartupAPI\User::createNewFacebookUser($name, $fbuser, $me);
		} catch (\StartupAPI\Exceptions\Authentication\UserCreationException $e) {
			$errors[$e->getField()][] = $e->getMessage();
		}

		if (count($errors) > 0) {
			throw new \StartupAPI\Exceptions\Authentication\ExistingUserException(
				'User already exists', 0, $errors
			);
		}
	...
	public function processEditUser($user, $data) {
		...
		if (!is_null(\StartupAPI\User::getUserByFacebookID($fbuser))) {
	...
	public function api(/* polymorphic */) {
		...
		try {
			...
			if (strpos($message, 'An active access token must be used') !== false ||
				...
				\StartupAPI\UserTools::debug('Facebook access token has expired, redirecting to login');
